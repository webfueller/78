...
is a large one-time cleanup rather than a subscription — see
[002](experiment-002.md) and [003](experiment-003.md) — and the machinery
underneath turned out to be the part that does not decay. Preflight is kept
because it is the proof the engine is not secretly mail-shaped, and because the
evidence it produced is what pointed the way.

Everything below is its own documentation, unchanged.
...
```bash
pip install -e . && pip install -e domains
```
